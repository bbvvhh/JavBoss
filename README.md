...
- **在线字幕**：播放器里按番号搜索字幕（默认接入迅雷字幕接口，地址可在设置中修改），搜索结果显示语言与字幕时长，
  下载后自动加载；一个字幕可以有多份，随视频一起保存在程序自己的 `data/subtitle` 目录并支持删除。设置里还能
  一键为全库 JAV 视频批量匹配字幕（优先按文件名匹配，同名多份时取时长最接近的）。电脑版与手机版都支持。
  用 MPV 播放时会自动带上已下载的字幕（mpv 里按 `j` / `J` 切换轨道），视频卡片上也有「字幕」入口可以搜索、下载、
  删除，并把选中的字幕立刻推送到正在播放的 MPV 窗口。
...
- **手机端独立界面**：同一份程序同时带电脑版与手机版两套前端。手机浏览器打开根路径会自动切到手机版（`/m/`），
  电脑访问照旧。手机版保留视频/JAV 浏览、搜索、播放、标签、收藏夹与全部设置项，并在设计上禁止任何会动到
  视频文件的危险操作（删除文件、目录整理）。想手动切换：手机看电脑版 `?desktop=1`，电脑看手机版 `?mobile=1`。
