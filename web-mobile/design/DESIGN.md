...
| P1 | **设置只留入口，功能不缩水** | 首屏无设置项；齿轮 →「我的」四组 14 个入口，覆盖 PC 端全部设置能力 |
...
│   │   ├── form/                # FormRow / Switch / Stepper / Segmented / TextField / PickerField
│   │   └── settings/            # MePage + 14 个子页
│   │       ├── MePage.jsx
│   │       ├── GlobalSettingsPage.jsx
│   │       ├── DirectoryManagerPage.jsx   # ⚠️ 无「整理 / 重命名」
│   │       ├── ScanTaskPage.jsx
│   │       ├── VideoTagPage.jsx
│   │       ├── JavTagPage.jsx
│   │       ├── FavoriteGroupPage.jsx
│   │       ├── PlayerSettingsPage.jsx
│   │       ├── ScrapeSettingsPage.jsx
│   │       ├── DownloaderPage.jsx
...
│   │       ├── AccountPage.jsx
│   │       └── AboutPage.jsx
│   ├── hooks/                   # useInfiniteScroll / useLongPress / usePullToRefresh / useUrlStateSync
│   └── utils/                   # 从 web/src/utils 复制（见 §6）
├── tests/
...
- 移动端「我的 → 切换到电脑版」：写 `javboss_ui=desktop` 后跳 `/`
...
| 文件 | 改动 |
| --- | --- |
| `internal/server/router.go` | `NewRouter` 增加 `mobileStaticDir`；注册 `/m/`；`NoRoute` 加 UA/Cookie 分流 |
| `internal/server/router_test.go` | 补 UA 分流、Cookie 覆盖、iPad 归移动端、`/m/` SPA fallback 测试 |
| `cmd/server/main.go` | 新增 `defaultMobileStaticDir = "web-mobile/dist"` |
| `internal/db/videos.go` | `hydrateLocationJavs` 加 `Preload("Idols")`（决策 1） |
| `internal/server/video_api.go` + `api.go` | 新增「从媒体库移除」DB-only 接口（见 §3.6） |
| `scripts/cli/cli.mjs` | release 流程增加移动端构建并拷贝 `web-mobile/dist` |
| `Dockerfile` | 增加移动端构建阶段并 `COPY` |
| `.gitignore` | 增加 `web-mobile/dist/`、`web-mobile/node_modules/` |
...
| **P3** | 后端 UA/Cookie 分流、`/m/` 托管、`Preload("Idols")`、`/videos/locations/hide`、构建发布接入、Go 测试 | ⏳ |
...
### P0 遗留（不影响使用，已在 README 标注）

- 演员行需要后端 `Preload("Idols")` 才能显示（P3），当前自动隐藏，加完无需改前端。
- JAV 页签与设置入口是占位页。
- `hideVideoLocations()` 已封装，但后端接口 P3 才落地，此前调用会 404 并如实报错。
...
1. **「从媒体库移除」的接口命名与形态**：当前设计是 `POST /videos/locations/hide` + `{location_ids: []}`。如果你更希望复用 `PATCH /videos/{id}/locations/{location_id}` 加 `{hidden:true}` 字段，也可以，但那个路由现在承载重命名，混用有误触发风险。
2. **P0 是否先不接后端切换**：P0 期间可以先用 `web-mobile` 的 dev server（5174 端口）独立调试，等 P3 再接 UA 分流。
